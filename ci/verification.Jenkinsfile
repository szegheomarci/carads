    ...
    stages {
        ...
        stage('Create config file') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: testdb-address, usernameVariable: 'HOST', passwordVariable: 'PORT',
                                usernamePassword(credentialsId: testdb-user, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD'),
                                string(credentialsId: testdb-dbname, variable: 'DBNAME')]) {
                        sh """
                            chmod +x test/createconfig.sh &&
                            test/createconfig.sh
                                \${HOST}
                                \${PORT}
                                \${USERNAME}
                                \${PASSWORD}
